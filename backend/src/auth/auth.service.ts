...
export class AuthService {
  ...
  async login({ loginDto }: { loginDto: LoginDto }) {
    try {
      ...
      if (accessToken && refreshToken) {
        const existingTokens = await this.prisma.userToken.findUnique({
          where: { userId: userWithoutPassword.uuid },
        });

        if (existingTokens) {
          newUserToken = await this.prisma.userToken.update({
            where: { userId: userWithoutPassword.uuid },
            data: {
          ...
          newUserToken = await this.prisma.userToken.create({
            data: {
              userId: userWithoutPassword.uuid,
              accessToken: accessToken,
              refreshToken: refreshToken,
  ...
  async activateAccount(activateAccountDto: ActivateAccountDto) {
    ...
    const validateAccount = await this.validateActivation({
      user,
      activationCode,
    });
  ...
  async verifiedResetPasswordToken({ token }: { token: string }) {
    try {
      ...
      if (!existingUser.isResettingPassword) {
        throw new HttpException(
          "Aucune demande de réinitialisation de mot de mot de passe n'est en cours.",
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  private async sendActivationCode({
    ...
    this.mailerService.sendActivationCodeEmail({
      recipient,
      firstname,
      activationCode,
    });
  ...
  private async validateActivation({
    ...
    if (
      user.activationCodeExpiredAt &&
      new Date(user.activationCodeExpiredAt).getTime() < Date.now()
    ) {
